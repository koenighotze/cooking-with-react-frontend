...
export default function RecipeEdit({recipe}) {
  const { handleRecipeSelected, handleRecipeChanged, handleIngredientAdd, handleIngredientDeleted } = useContext(RecipeContext)
  ...
  return (
    <div className='recipe-edit'>
      <div className='recipe-edit__remove-button-container'>
        <button className='btn recipe-edit__remove-button' onClick={() => handleRecipeSelected(null)}>&times;</button>
      </div>
      <div className='recipe-edit__details-grid'>
        ...
        <input className='recipe-edit__input' type="number" min="1" name="servings" id="servings" value={servings} onInput={(e) => handleChange({ servings: parseInt(e.target.value) || ''})}/>
      ...
      <div  className='recipe-edit__ingredient-grid'>
        ...
        <div>Amount</div>
        <div></div>
        {ingredients.map(i => <RecipeIngredientEdit key={i.id} ingredient={i} handleIngredientChanged={handleIngredientChanged} handleIngredientDeleted={handleIngredientDeleted}/>)}
      </div>
